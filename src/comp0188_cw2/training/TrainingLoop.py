...
def train(
    ...
    mo.add_metric(nm="epoch_train_loss", rll_trans={})
    mo.add_metric(nm="epoch_val_loss", rll_trans={})
    ...
    for epoch in np.arange(1, epochs + 1):
        ...
        epoch_train_loss = train_loss_val.detach().cpu().numpy()  # Detach and move to CPU
        ...
        epoch_val_loss = val_loss_val.detach().cpu().numpy()  # Detach and move to CPU
        logger.info("Running validation")
        logger.info("epoch {}\t validation loss : {} ".format(epoch, epoch_val_loss))

        mo.update_metrics(metric_value_dict={
            "epoch_train_loss": {"label": "epoch_{}".format(epoch), "value": epoch_train_loss},
            "epoch_val_loss": {"label": "epoch_{}".format(epoch), "value": epoch_val_loss}
        })

        if scheduler:
            scheduler.step()

        chkp_pth = os.path.join(save_dir, "mdl_chkpnt_epoch_{}.pt".format(epoch))
        torch.save(
            {
                'epoch': epoch,
        ...
        if preds_save_type is not None:
            if preds_save_type == "pickle":
                for k in train_preds.keys():
                    with open(
                        os.path.join(output_dir, f"epoch_{epoch}_train_preds_{k}.pkl"), "wb"
                    ) as file:
                        pickle.dump(train_preds[k], file)
                for k in val_preds.keys():
                    with open(
                        os.path.join(output_dir, f"epoch_{epoch}_val_preds_{k}.pkl"), "wb"
                    ) as file:
                ...
                for k in train_preds.keys():
                    np.savetxt(
                        os.path.join(output_dir, f"epoch_{epoch}_train_preds_{k}.csv"),
                        train_preds[k].detach().cpu().float().numpy(),  # Detach and move to CPU
                        delimiter=","
                    )
                for k in val_preds.keys():
                    np.savetxt(
                        os.path.join(output_dir, f"epoch_{epoch}_val_preds_{k}.csv"),
                        val_preds[k].detach().cpu().float().numpy(),  # Detach and move to CPU
                        delimiter=","
                    )
            else:
                raise ValueError(
...
class TorchTrainingLoop:
    ...
    def __init__(
        ...
        """Class to orchestrate training and validating a model."""
        self.model = model
